...
void wilson_gen(mbit *mz, const std::size_t mz_w, const std::size_t mz_h,
  ...
  while (cells_left) {
    ...
    while (!getcell(partof_mz, crrnt_pos)) {
      ...
      prnt_pos = crrnt_pos;
      switch (
          dirs_pos[(std::uniform_int_distribution<int>(0, dirs_top - 1))(mt)]) {
      case 0:
        crrnt_pos += mz_w;
        break;
      case 1:
      ...
      if (tmp_strg.find(crrnt_pos) != tmp_strg.end()) {
        ...
        if (crrnt_pos < ccount - mz_w &&
            tmp_strg.find(crrnt_pos + mz_w) != tmp_strg.end() &&
            tmp_strg[crrnt_pos + mz_w] == crrnt_pos)
          prnt_pos = crrnt_pos + mz_w;
        else if (crrnt_pos % mz_w != mz_w - 1 &&
                 tmp_strg.find(crrnt_pos + 1) != tmp_strg.end() &&
                 tmp_strg[crrnt_pos + 1] == crrnt_pos)
          prnt_pos = crrnt_pos + 1;
        else if (crrnt_pos >= mz_w &&
                 tmp_strg.find(crrnt_pos - mz_w) != tmp_strg.end() &&
                 tmp_strg[crrnt_pos - mz_w] == crrnt_pos)
          prnt_pos = crrnt_pos - mz_w;
        else if (crrnt_pos % mz_w != 0 &&
                 tmp_strg.find(crrnt_pos - 1) != tmp_strg.end() &&
                 tmp_strg[crrnt_pos - 1] == crrnt_pos)
          prnt_pos = crrnt_pos - 1;
        else
